...
{
    RANGE product = 1; //淇濆瓨涔樼Н銆備箻娉曡繍绠楃殑鍖栫畝鏄檷浣嶰(T(N))鐨勫繀鐢变箣璺�
    char smallest_bit; //浠庢渶浣庝綅寮€濮嬶紝鍚戦珮浣嶆彁鍙栵紝閫愪竴鍚戝彸绉诲姩鎵€鏈変綅锛岄亣鍒�0鐩存帴璺冲嚭寰幆

    //鍒嗚В浼犲叆鐨勭礌鏁�
    while (prime != 0)
    {
        smallest_bit = prime % 10; //鍙栨渶灏忎綅
#ifdef USE_FAST_PRODUCTION
        if (smallest_bit == 0)
            return 0; //鑰冭檻瀹為檯鎰忎箟锛岀洿鎺ヨ繑鍥炲嵆鍙紱1涓嶇敤鏀瑰彉product鐨勫€�
        //core
        if (smallest_bit > 2)
...
{
    Sheldon *inter = arg;
    RANGE index, primenumber = inter->initialPn; //閬嶅巻鍙傛暟锛岀n涓礌鏁�
    int sheldonflag = 0;                         //鏄惁涓篠heldon绱犳暟鐨勬爣蹇�
